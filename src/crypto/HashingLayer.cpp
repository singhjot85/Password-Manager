...
string HashingLayer::hashWithSalt(const string& sensitiveData, const string& salt) const {
    // Dependency-free first implementation. Replace with Argon2, bcrypt, or
    // PBKDF2 before handling real production credentials.
...
bool HashingLayer::parseHash(const string& storedHash, string& salt, string& hashValue) const {
    ...
    if (storedHash.substr(0, versionSeparator) != HASH_VERSION) {
        return false;
    }
